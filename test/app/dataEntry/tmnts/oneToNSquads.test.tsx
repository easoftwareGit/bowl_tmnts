  ...
  describe('updatedLanes(lanes, squad', () => { 
    ...
    describe('updatedLanes - one squad', () => { 
      ...
      it('updatedLanes - one squad - lanes values', () => {
        const updated = updatedLanes(testLanes, testSquads[0]);
        expect(updated[0].lane).toEqual(testSquads[0].starting_lane);
        expect(updated[testSquads[0].lane_count - 1].lane).toEqual(testSquads[0].starting_lane + testSquads[0].lane_count - 1);
        expect(updated[testSquads[0].lane_count - 1].lane).toEqual(10);
      })
      it('updatedLanes - one squad - change starting lane', () => {        
        testSquads[0].starting_lane = 5;
        const updated = updatedLanes(testLanes, testSquads[0]);
        expect(updated[0].lane).toEqual(testSquads[0].starting_lane);
        expect(updated[0].lane).toEqual(5);
        expect(updated[testSquads[0].lane_count - 1].lane).toEqual(testSquads[0].starting_lane + testSquads[0].lane_count - 1);
        expect(updated[testSquads[0].lane_count - 1].lane).toEqual(14);
      })
      it('updatedLanes - one squad - change lane count', () => {        
        testSquads[0].lane_count = 12;
        const updated = updatedLanes(testLanes, testSquads[0]);
        expect(updated[0].lane).toEqual(testSquads[0].starting_lane);
        expect(updated[0].lane).toEqual(1);
        expect(updated[testSquads[0].lane_count - 1].lane).toEqual(testSquads[0].starting_lane + testSquads[0].lane_count - 1);
        expect(updated[testSquads[0].lane_count - 1].lane).toEqual(12);
      })
      it('updatedLanes - one squad - no lanes', () => {
        ...
        expect(updated[0].lane).toEqual(1);
        expect(updated[9].lane).toEqual(10);
      })
    })
    describe('updatedLanes - two squads', () => { 
      ...
      it('updatedLanes - one squad - change starting lane on squad 1', () => {        
        ...
        expect(updated[0].lane).toEqual(testSquads[1].starting_lane);
        expect(updated[0].lane).toEqual(1);
        expect(updated[testSquads[1].lane_count - 1].lane).toEqual(testSquads[1].starting_lane + testSquads[1].lane_count - 1);        
        expect(updated[testSquads[1].lane_count - 1].lane).toEqual(10);
        expect(updated[testSquads[1].lane_count].lane).toEqual(testSquads[0].starting_lane);
        expect(updated[testSquads[1].lane_count].lane).toEqual(5);
        expect(updated[updated.length - 1].lane).toEqual(testSquads[0].starting_lane + testSquads[0].lane_count - 1);
        expect(updated[updated.length-1].lane).toEqual(24);
      })
      it('updatedLanes - one squad - change starting lane on squad 1', () => { 
        ...
        expect(updated[0].lane).toEqual(1);
        expect(updated[1].lane).toEqual(2);
        expect(updated[2].lane).toEqual(1);
        expect(updated[3].lane).toEqual(2);
        expect(updated[updated.length - 1].lane).toEqual(20);
      })       
    })
