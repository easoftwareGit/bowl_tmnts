/*
  ...
  - You are about to drop the column `entry_fee` on the `Se_Div_Feat` table. All the data in the column will be lost.
  - Added the required column `fee` to the `Se_Div_Feat` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE "Me_Div_Feat" ALTER COLUMN "id" SET DEFAULT concat('dvf_', replace(cast(gen_random_uuid() as text), '-', ''));

-- AlterTable
ALTER TABLE "Player" ALTER COLUMN "id" SET DEFAULT concat('ply_', replace(cast(gen_random_uuid() as text), '-', ''));

-- AlterTable
ALTER TABLE "Se_Div_Feat" DROP COLUMN "entry_fee",
