/*
  ...
  - You are about to drop the column `lane` on the `Lanes` table. All the data in the column will be lost.
  - Added the required column `lane_number` to the `Lanes` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE "Lanes" DROP COLUMN "lane",
ADD COLUMN     "lane_number" INTEGER NOT NULL,
ALTER COLUMN "id" SET DEFAULT concat('lns_', replace(cast(gen_random_uuid() as text), '-', ''));
