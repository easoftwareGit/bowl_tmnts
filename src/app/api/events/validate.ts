...
export const validTeamSize = (teamSize: number): boolean => {
  ...
  return Number.isInteger(teamSize) &&
...
export const entryFeeEqualsLpox = (event: eventType): boolean => { 
  ...
  try {
    ...
    if (isNaN(entryFee) || isNaN(lineage) || isNaN(prizeFund) || isNaN(other) || isNaN(expenses) || isNaN(lpox)) {
      return false
    }
    return (entryFee === lineage + prizeFund + other + expenses
      && entryFee === lpox)
  } catch (error) {
    return false
  }
