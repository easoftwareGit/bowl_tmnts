...
import { validUserEmail } from "../api/users/validate";
  ...
  const potCreate = async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        const postedDiv: potType = response.data.pot;
        if (postedDiv.div_id !== potToPost.div_id) {
          testResults += addToResults("Created pot div_id !== divToPost.div_id", false);
        } else if (postedDiv.squad_id !== potToPost.squad_id) {
          testResults += addToResults(
            "Created pot squad_id !== divToPost.squad_id",
            false
          );
        } else if (postedDiv.pot_type !== potToPost.pot_type) {
          testResults += addToResults(
            "Created pot div_name !== divToPost.pot_type",
            false
          );
        } else if (postedDiv.fee !== potToPost.fee) {
          testResults += addToResults(
            "Created pot fee !== divToPost.fee",
            false
          );
        } else if (postedDiv.sort_order !== potToPost.sort_order) {
          testResults += addToResults(
            "Created pot sort_order !== divToPost.sort_order",
            false
          );
        } else {
          testResults += addToResults(`Created pot === divToPost`);
        }
      } else {
        testResults += addToResults(
          `Error creating pot: ${potToPost.pot_type}, response statue: ${response.status}`,
          false
        );
        return {
      ...
      await invalidCreate("tmnt_id", "bwl_123");
      await invalidCreate("div_name", "");
      await invalidCreate("hdcp_per", 2);
      await invalidCreate("hdcp_from", -1);
      await invalidCreate("int_hdcp", null);
      await invalidCreate("hdcp_for", "Test");
      ...
      if (createdPotId) {
        await potDelete(createdPotId, false);
      }
      if (passed) {
  ...
  const potReadAll = async (showResults: boolean) => {
    ...
    try {
      ...
      if (response.status === 200) {
        if (showResults) {
          testResults += addToResults(
            `Success: Read ${response.data.pots.length} Events`,
            true
          );
        }
        const all: potType[] = response.data.pots as unknown as potType[];
        ...
        testResults += addToResults(
          `Error reading all pots, response statue: ${response.status}`,
          false
        );
        return {
  ...
  const potRead1 = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        testResults += addToResults(
          `Error reading 1 pot, response statue: ${response.status}`,
          false
        );
        return {
      ...
      await readInvalidId("div_12345678901234567890123456789012");
    ...
    const validReadForDiv = async (divId: string) => {
      try {
        ...
        if (response.status === 200) {
          ...
          testResults += addToResults(
            `Error reading pots for div, response statue: ${response.status}`,
            false
          );
          return {
    ...
    const validReadForSquad = async (squadId: string) => {
      try {
        ...
        if (response.status === 200) {
          ...
          testResults += addToResults(
            `Error reading pots for squad, response statue: ${response.status}`,
            false
          );
          return {
      ...
      if (response.status === 200) {
        ...
        if (potIdToDel === potToDel.id) {
          if (response.data.deleted.pot_type === potToDel.pot_type) {
            testResults += addToResults(
              `Success: Deleted Pot: ${potToDel.pot_type} - got hdcp_per_str in response`
            );
          } else {
            testResults += addToResults(
