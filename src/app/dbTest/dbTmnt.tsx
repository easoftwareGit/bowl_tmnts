...
export const DbTmnts = () => {
  ...
  const removeCreatedTmnt = async (showResults: boolean) => {
    ...
    try {
      const allTmnts: tmntType[] = await tmntReadAll(false) as unknown as tmntType[]
      const justPostedTmnt = allTmnts.filter(tmnt => tmnt.tmnt_name === tmntToPost.tmnt_name);
      if (justPostedTmnt.length === 1) {
        tmntDelete(justPostedTmnt[0].id, false)
        if (showResults) {
          testResults += addToResults(`Reset Created Tmnt: ${justPostedTmnt[0].tmnt_name}`);
        }        
      }      
      return allTmnts
    } catch (error: any) {
  ...
  const reAddDeletedTmnt = async () => {
    ...
    try {
      ...
      try {
        ...
        if (response.status === 200 && (response.data.tmnt || response.data.tmnts?.length > 0)) {       
          return {
            data: tmntToDel,
            status: 201
          }
  ...
  const tmntCreate = async () => {
    ...
    const tmntInvalidCreate = async (propertyName: string, value: any) => { 
      try {        
        ...
        if (invalidResponse.status !== 422) {
          testResults += addToResults(`Create Tmnt Error: did not return 422 for invalid ${propertyName}`, false)
          // setResults(testResults)
          return {
          ...
          testResults += addToResults(`Create Tmnt, non 422 response for tmnt: ${tmntToUpdate.tmnt_name} - invalid data`)
          return {
        ...
        if (error.response.status === 422) {
          testResults += addToResults(`DID NOT Create Tmnt: ${tmntToUpdate.tmnt_name} - invalid ${propertyName}`)          
          return {
            error: '',
            status: error.response.status,
          }
    ...
    try {      
      ...
      if (response.status === 201) {
        testResults += addToResults(`Created Tmnt: ${response.data.tmnt.tmnt_name}`)
        const postedTmnt: tmntType = response.data.tmnt;
        if (postedTmnt.tmnt_name !== tmntToPost.tmnt_name) {
          testResults += addToResults('Created Tmnt tmnt_name !== tmntToPost.tmnt_name', false)
        } else if (postedTmnt.start_date !== tmntToPost.start_date) {
          testResults += addToResults('Created Tmnt start_date !== tmntToPost.start_date', false)
        } else if (postedTmnt.end_date !== tmntToPost.end_date) {
          testResults += addToResults('Created Tmnt end_date !== tmntToPost.end_date', false)
        } else if (postedTmnt.bowl_id !== tmntToPost.bowl_id) {
          testResults += addToResults('Created Tmnt bowl_id !== tmntToPost.bowl_id', false)
        } else if (postedTmnt.user_id !== tmntToPost.user_id) {
          testResults += addToResults('Created Tmnt user_id !== tmntToPost.user_id', false)
        } else {
          testResults += addToResults(`Created Tmnt === tmntToPost`)
        }
        createdTmntId = response.data.tmnt.id;
      } else {
        testResults += addToResults(`Error creating tmnt: ${tmntToPost.tmnt_name}, response statue: ${response.status}`, false);        
        // setResults(testResults)
        return {
  ...
  const tmntReadAll = async (showResults: boolean) => {    
    ...
    try {
      ...
      if (response.status === 200) {
        if (showResults) {
          testResults += addToResults(`Success: Read ${response.data.tmnts.length} Tmnts`, true);
          // setResults(testResults)        
        }
        const allTmnts: tmntType[] = response.data.tmnts as unknown as tmntType[]
        const justPostedTmnt = allTmnts.filter(tmnt => tmnt.tmnt_name === tmntToPost.tmnt_name);
        ...
        if (justPostedTmnt.length === 1) { 
          ...
          if (allTmnts.length === seedTmnts + 1) { 
            ...
            testResults += addToResults(`Error: Read ${allTmnts.length} tmnts, expected ${seedTmnts + 1}`, false);
          }
        } else {
          ...
          if (allTmnts.length === seedTmnts) {
            ...
            testResults += addToResults(`Error: Read ${allTmnts.length} tmnts, expected ${seedTmnts}`, false);
          }          
        }
        return response.data.tmnts;
      } else {
        testResults += addToResults(`Error reading all tmnts, response statue: ${response.status}`, false);        
        // setResults(testResults)
        return {
  ...
  const tmntRead1 = async () => {
    ...
    const tmntReadInvalidId = async (id: string) => {
      try {
        ...
        if (invalidResponse.status !== 404) {
          testResults += addToResults(`Read 1 Tmnt Error: did not return 404 for invalid id ${id}`, false)
          // setResults(testResults)        
          return {
          ...
          testResults += addToResults(`Read 1 Tmnt, non 404 response for invalid id: ${id}`)
          return {
    ...
    try {
      ...
      if (response.status === 200) {        
        testResults += addToResults(`Success: Read 1 Tmnt ${response.data.tmnt.tmnt_name}`, true);        
        const readTmnt: tmntType = response.data.tmnt;
        if (readTmnt.tmnt_name !== testTmnt.tmnt_name) {
          testResults += addToResults('Read 1 Tmnt tmnt_name !== testTmnt.tmnt_name', false)
        } else if (readTmnt.start_date !== testTmnt.start_date) {          
          testResults += addToResults('Read 1 Tmnt start_date !== testTmnt.start_date', false)
        } else if (readTmnt.end_date !== testTmnt.end_date) {
          testResults += addToResults('Read 1 Tmnt end_date !== testTmnt.end_date', false)
        } else if (readTmnt.bowl_id !== testTmnt.bowl_id) {
          testResults += addToResults('Read 1 Tmnt bowl_id !== testTmnt.bowl_id', false)
        } else if (readTmnt.user_id !== testTmnt.user_id) {
          testResults += addToResults('Read 1 Tmnt user_id !== testTmnt.user_id', false)
        } else {
          testResults += addToResults(`Read 1 Tmnt === testTmnt`)
        }        
      } else {
        testResults += addToResults(`Error reading 1 Tmnt, response statue: ${response.status}`, false);        
        // setResults(testResults)
        return {
  ...
  const tmntUpdate = async () => {
    ...
    const tmntUpdateInvalidId = async (id: string) => {      
      try {
        ...
        if (error.response.status === 404) {
          ...
          testResults += addToResults(`Update Tmnt Error: did not return 404 for invalid id: ${id}`, false)
          // setResults(testResults)        
          return {
    ...
    try {
      ...
      if (updatedTmnt.tmnt_name !== tmntUpdatedTo.tmnt_name) {
        testResults += addToResults('Updated Tmnt tmnt_name !== testTmntToUpdate.tmnt_name', false)
      } else if (updatedTmnt.start_date !== tmntUpdatedTo.start_date) {
        testResults += addToResults('Updated Tmnt start_date !== testTmntToUpdate.start_date', false)
      } else if (updatedTmnt.end_date !== tmntUpdatedTo.end_date) {
        testResults += addToResults('Updated Tmnt end_date !== testTmntToUpdate.end_date', false)
      } else if (updatedTmnt.bowl_id !== tmntUpdatedTo.bowl_id) {
        testResults += addToResults('Updated Tmnt bowl_id !== testTmntToUpdate.bowl_id', false)
      } else if (updatedTmnt.user_id !== tmntUpdatedTo.user_id) {
        testResults += addToResults('Updated Tmnt user_id !== testTmntToUpdate.user_id', false)
      } else {
      ...
      await tmntInvalidCreate('bowl_id', 'usr_12345678901234567890123456789012');
      await tmntInvalidCreate('user_id', 'bwl_12345678901234567890123456789012');
  ...
  const tmntPatch = async () => {
    ...
    const doPatchTmnt = async (propertyName: string, value: any, matchValue: any) => {
      try {
        let tmntJSON: any;
        ...
        if (response.status === 200) {          
          if (response.data.tmnt[propertyName] === matchValue) { 
            if (propertyName === 'start_date' && value === '') { 
              testResults += addToResults(`Patched Tmnt: ${tmntToUpdate.tmnt_name} - just ${propertyName} = "${value}" and end_date = ""`) 
            } else {
              testResults += addToResults(`Patched Tmnt: ${tmntToUpdate.tmnt_name} - just ${propertyName} = "${value}"`)            
            }
          } else {
            testResults += addToResults(`DID NOT Patch Tmnt ${propertyName}`, false)
            // setResults(testResults)
          }
          return {
        ...
        testResults += addToResults(`doPatchTmnt Error: ${error.message}`, false);
        // setResults(testResults)
        return {
          error: error.message,
          status: 404,
    ...
    const doNotPatchTmnt = async (propertyName: string, value: any) => {       
      try {
        const tmntJSON = JSON.stringify({
          ...tmntToUpdate,
          tmnt_name: "123456789012345678901234567890123456789012345678901234567890",
        })
        const response = await axios({
          method: "patch",
          data: tmntJSON,
        ...
        if (response.status !== 422) {
          ...
          testResults += addToResults(`Patch Tmnt, non 422 response for tmnt: ${tmntToUpdate.tmnt_name} - invalid ${propertyName}`)
          return {
            error: 'Error Patching Tmnt',
            status: response.status,
          };          
        ...
        if (error.response.status === 422) {
          testResults += addToResults(`DID NOT Patch Tmnt: ${tmntToUpdate.tmnt_name} - invalid ${propertyName}`)          
          return {
            error: '',
            status: error.response.status,
          }
    ...
    const tmntPatchInvalidId = async (id: string) => {      
      try {
        ...
        if (error.response.status === 404) {
          ...
          testResults += addToResults(`Patch Tmnt Error: did not return 404 for invalid id: ${id}`, false)
          // setResults(testResults)        
          return {
    ...
    try {      
      ...
      await doPatchTmnt('bowl_id', 'bwl_8b4a5c35ad1247049532ff53a12def0a', 'bwl_8b4a5c35ad1247049532ff53a12def0a')
      await doNotPatchTmnt('bowl_id', 'usr_8b4a5c35ad1247049532ff53a12def0a')

      await doPatchTmnt('user_id', 'usr_a24894ed10c5dd835d5cbbfea7ac6dca', 'usr_a24894ed10c5dd835d5cbbfea7ac6dca')
    ...
    const invalidDelete = async (invalidId: string) => {      
      try {
        ...
        if (cantDelResponse.status === 404) {
          testResults += addToResults(`Did not not delete tmnt with invalid id: "${invalidId}"`)
        } else {
          testResults += addToResults(`Error: Could not delete tmnt with invalid id: "${invalidId}"`, false)
        }
      } catch (error: any) {
        if (error.response.status === 404) {
          testResults += addToResults(`Did not not delete tmnt - invalid id: "${invalidId}"`)
        } else {
          testResults += addToResults(`Delete Tmnt Error: ${error.message}`, false);
          // setResults(testResults)
          return {
            error: error.message,
            status: error.response.status,
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (tmntId === tmntToDel.id) {
          testResults += addToResults(`Success: Deleted Tmnt: ${response.data.deleted.tmnt_name}`);          
        }        
      } else {
      ...
      if (testing) {
        ...
        try {
          ...
          if (cantDelResponse.status === 409) {
            testResults += addToResults(`Did not not delete tmnt: ${tmntToUpdate.tmnt_name} with children`)
          } else {
            testResults += addToResults(`Error: Could not delete tmnt: ${tmntToUpdate.tmnt_name}`, false)
          }
        } catch (error: any) {
  ...
  const resetAll = async () => { 
    ...
    try {
      ...
      if (allTmnts.error) {
        testResults += addToResults(`Error Resetting: ${allTmnts.error}`, false)
        setResults(testResults)
        return;
      }
