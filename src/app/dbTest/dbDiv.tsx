...
export const DbDivs = () => {
  ...
  const divCreate = async () => {
    ...
    const divInvalidCreate = async (propertyName: string, value: any) => { 
      try {        
        ...
        if (invalidResponse.status !== 422) {
          testResults += addToResults(`Create Div Error: did not return 422 for invalid ${propertyName}`, false)          
          return {
            error: `Error creating div with invalid ${propertyName}`,
            status: invalidResponse.status,
          };
        } else {
          testResults += addToResults(`Create Div, non 422 response for div: ${divToUpdate.div_name} - invalid data`)
          return {
        ...
        if (error.response.status === 422) {
          testResults += addToResults(`DID NOT Create div: ${divToUpdate.div_name} - invalid ${propertyName}`)
          return {
            error: '',
            status: error.response.status,
          }
    ...
    const divCreateDublicate = async () => {
      try {
        const duplicate = {
          ...divDuplicate,
          id: ''
        ...
        if (invalidResponse.status !== 422) {
          testResults += addToResults(`Create Div Error: did not return 422 for duplicate div_name+tmnt_id`, false)          
          return {
            error: `Error creating div with duplicate div_name+tmnt_id`,
            status: invalidResponse.status,
          };
        } else {
          testResults += addToResults(`Create Div, non 422 response for duplicate div_name+tmnt_id`)
          return {
        ...
        if (error.response.status === 422) {
          ...
          testResults += addToResults(`Create Error: did not return 422 for duplicate div_name+tmnt_id`, false)
          return {
            error: `Error Creating div with duplicate div_name+tmnt_id`,
            status: error.response.status,
          };          
    ...
    try {      
      ...
      if (response.status === 201) {
        ...
        if (postedDiv.tmnt_id !== divToPost.tmnt_id) {
          testResults += addToResults('Created div tmnt_id !== divToPost.city', false)
        } else if (postedDiv.div_name !== divToPost.div_name) {
          testResults += addToResults('Created div div_name !== divToPost.div_name', false)        
        } else if (postedDiv.hdcp_per !== divToPost.hdcp_per) {
          testResults += addToResults('Created div hdcp_per !== divToPost.hdcp_per', false)          
        } else if (postedDiv.hdcp_from !== divToPost.hdcp_from) {
          testResults += addToResults('Created div hdcp_from !== divToPost.hdcp_from', false)
        } else if (postedDiv.int_hdcp !== divToPost.int_hdcp) {
          testResults += addToResults('Created div int_hdcp !== divToPost.int_hdcp', false)
        } else if (postedDiv.hdcp_for !== divToPost.hdcp_for) {
          testResults += addToResults('Created div hdcp_for !== divToPost.hdcp_for', false)
  ...
  const divReadAll = async (showResults: boolean) => {    
    ...
    try {
      ...
      if (response.status === 200) {
        if (showResults) {
          testResults += addToResults(`Success: Read ${response.data.divs.length} Events`, true);                  
        }
        ...
        const seedDivs = 7
        if (justPostedDiv.length === 1) { 
          // created a test div BEFORE testing read all
          if (allDivs.length === seedDivs + 1) { 
            testResults += addToResults(`Read all ${seedDivs + 1} divs`);
          } else {
            testResults += addToResults(`Error: Read ${allDivs.length} divs, expected ${seedDivs + 1}`, false);
          }
        } else {
          // test div not created yet
          if (allDivs.length === seedDivs) {
            testResults += addToResults(`Read all ${seedDivs} divs`, true);
          } else {
            testResults += addToResults(`Error: Read ${allDivs.length} divs, expected ${seedDivs}`, false);
          }          
        }

        return response.data.divs;
      } else {
        testResults += addToResults(`Error reading all divs, response statue: ${response.status}`, false);        
        return {
  ...
  const divRead1 = async () => {
    ...
    const divReadInvalidId = async (id: string) => {
      try {
        ...
        if (invalidResponse.status !== 404) {
          testResults += addToResults(`Read 1 Div Error: did not return 404 for invalid id ${id}`, false)                  
          return {
            error: `Error getting with invalid id: ${id}`,
            status: invalidResponse.status,
          };          
        } else {
          testResults += addToResults(`Read 1 Div, non 404 response for invalid id: ${id}`)
          return {
    ...
    try {
      ...
      if (response.status === 200) {        
        testResults += addToResults(`Success: Read 1 Div: ${response.data.div.div_name}`, true);        
        ...
        if (readDiv.tmnt_id !== testDiv.tmnt_id) {
          ...
          testResults += addToResults('Read 1 Div hdcp_from !== testDiv.hdcp_from', false)
          ...
          testResults += addToResults('Read 1 Div sort_order !== testDiv.sort_order', false)        
        } else {
          testResults += addToResults(`Read 1 Div === testDiv`)
        }        
      } else {
        testResults += addToResults(`Error reading 1 div, response statue: ${response.status}`, false);        
        return {
  ...
  const divReadForTmnt = async () => {
        ...
        if (response.status === 200) {
          ...
          if (tmntId === multiDivsTmntId) {
            if (readDivs.length !== 2) {
              testResults += addToResults('Error: Read Divs for Tmnt length !== 2', false)
              return {
            ...
            readDivs.forEach((div: divType) => {
              if (!(div.id === 'div_1f42042f9ef24029a0a2d48cc276a087'
                || div.id === 'div_29b9225d8dd44a4eae276f8bde855729')) {
                testResults += addToResults('Error: Read Divs for Tmnt div.id invalid', false)                
                return {
            ...
            if (readDivs.length !== 0) {
              testResults += addToResults('Error: Read Divs for Tmnt length !== 0', false)              
              return {
          ...
          testResults += addToResults(`Success: Read Divs for Tmnt, ${readDivs.length} rows returned`)
        } else {
          testResults += addToResults(`Error reading divs for tmnt, response statue: ${response.status}`, false);          
          return {
    ...
    const invalidReadForTmnt = async (tmntId: string) => {
      try {
        ...
        if (invalidResponse.status !== 404) {
          testResults += addToResults(`Read Divs for Tmnt Error: did not return 404 for invalid id ${tmntId}`, false)                  
          return {
            error: `Error getting with invalid id: ${tmntId}`,
            status: invalidResponse.status,
          };          
        } else {
          testResults += addToResults(`Read Divs for Tmnt, non 404 response for invalid id: ${tmntId}`)
          return {
        ...
        if (error.response.status === 404) {
          testResults += addToResults(`DID NOT Read Divs for Tmnt: invalid id: ${tmntId}`)          
          return {
  ...
  const divUpdate = async () => {
    ...
    const divUpdateValid = async () => {
      try {
        const divJSON = JSON.stringify(divUpdatedTo);
        const response = await axios({
          method: "put",
          data: divJSON,
          withCredentials: true,
        ...
        const updatedDiv: divType = response.data.div;
        if (updatedDiv.tmnt_id !== divUpdatedTo.tmnt_id) {
          testResults += addToResults('Updated div tmnt_id !== divUpdatedTo.tmnt_id', false)
        } else if (updatedDiv.div_name !== divUpdatedTo.div_name) {
          testResults += addToResults('Updated div div_name !== divUpdatedTo.div_name', false)
        } else if (updatedDiv.hdcp_per !== divUpdatedTo.hdcp_per) {
          testResults += addToResults('Updated div hdcp_per !== divUpdatedTo.hdcp_per', false)
        } else if (updatedDiv.hdcp_from !== divUpdatedTo.hdcp_from) {
          testResults += addToResults('Updated div hdcp_from !== divUpdatedTo.hdcp_from', false)
        } else if (updatedDiv.int_hdcp !== divUpdatedTo.int_hdcp) {
          testResults += addToResults('Updated div int_hdcp !== divUpdatedTo.int_hdcp', false)
        } else if (updatedDiv.hdcp_for !== divUpdatedTo.hdcp_for) {
          testResults += addToResults('Updated div hdcp_for !== divUpdatedTo.hdcp_for', false)
        } else if (updatedDiv.sort_order !== divUpdatedTo.sort_order) {
          testResults += addToResults('Updated div sort_order !== divUpdatedTo.sort_order', false)  
        } else {
          testResults += addToResults(`Updated Div: ${updatedDiv.div_name}`);
        }                
        return response;
      } catch (error: any) {
    ...
    const divInvalidUpdate = async (propertyName: string, value: any) => { 
      try {        
        const invalidDivJSON = JSON.stringify({
          ...divToUpdate,
          [propertyName]: value,
        ...
        if (invalidResponse.status !== 422) {
          testResults += addToResults(`Update Div Error: did not return 422 for invalid ${propertyName}`, false)          
          return {
            error: `Error updating div with invalid ${propertyName}`,
            status: invalidResponse.status,
          };
        } else {
          testResults += addToResults(`Update Div, non 422 response for div: ${divToUpdate.div_name} - invalid data`)
          return {
        ...
        if (error.response.status === 422) {
          testResults += addToResults(`DID NOT Update div: ${divToUpdate.div_name} - invalid ${propertyName}`)
          return {
            error: '',
            status: error.response.status,
          }
    ...
    const divUpdateInvalidId = async (id: string) => {      
      try {
        const invalidUrl = url + "/" + id;
        const tmntJSON = JSON.stringify(divUpdatedTo);
        ...
        if (error.response.status === 404) {
          ...
          testResults += addToResults(`Update Div Error: did not return 404 for invalid id: ${id}`, false)                  
          return {
    ...
    const divDontUpdateDuplicate = async () => {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          testResults += addToResults(`DID NOT update Div, duplicate div_name+tmnt_id`, false);
        }
        return response;
      } catch (error: any) {
        if (error.response.status === 422) {
          ...
          testResults += addToResults(`Update Div Error: did not return 422 for duplicate div_name+tmnt_id`, false)                 
          return {
    ...
    try {
      ...
      const updated = await divUpdateValid();

      // 2) invalid Div object
      await divInvalidUpdate('tmnt_id', 'bwl_123');
      await divInvalidUpdate('div_name', '');
      await divInvalidUpdate('hdcp_per', 2);
      await divInvalidUpdate('hdcp_from', 301);      
      await divInvalidUpdate('int_hdcp', undefined);
      await divInvalidUpdate('hdcp_for', '');
      await divInvalidUpdate('sort_order', 'abc');

      // 3) invalid Div id
      await divUpdateInvalidId('abc_123');
      // 4 non existing Div id
      await divUpdateInvalidId('div_12345678901234567890123456789012');

      // 5) duplicate Div
      await divDontUpdateDuplicate();
  ...
  const divPatch = async () => {
    ...
    const doPatchDiv = async (propertyName: string, value: any, matchValue: any) => {

      try {
        const divJSON = JSON.stringify({          
            [propertyName]: value,
        })        
        const response = await axios({
          method: "patch",
        ...
        if (response.status === 200) {          
          if (response.data.div[propertyName] === matchValue) {
            testResults += addToResults(`Patched Div: ${divToUpdate.div_name} - just ${propertyName}`)                        
          } else {
        ...
        testResults += addToResults(`doPatchDiv Error: ${error.message}`, false);        
        return {
          error: error.message,
          status: 404,
        };
    ...
    const doNotPatchDiv = async (propertyName: string, value: any) => {       
      try {
        const divJSON = JSON.stringify({
          ...divToUpdate,
          div_name: "123456789012345678901234567890123456789012345678901234567890",
        })
        const response = await axios({
          method: "patch",
          data: divJSON,
        ...
        if (response.status !== 422) {
          ...
          testResults += addToResults(`Patch Div, non 422 response for div: ${divToUpdate.div_name} - invalid ${propertyName}`)
          return {
            error: 'Error Patching Event',
            status: response.status,
          };          
        ...
        if (error.response.status === 422) {
          testResults += addToResults(`DID NOT Patch Div: ${divToUpdate.div_name} - invalid ${propertyName}`)
          return {
            error: '',
            status: error.response.status,
          }
    ...
    const divPatchInvalidId = async (id: string) => {      
      try {
        const invalidUrl = url + "/" + id;
        const tmntJSON = JSON.stringify(divUpdatedTo);
        const notUpdatedResponse = await axios({
        ...
        if (error.response.status === 404) {
          ...
          testResults += addToResults(`Patch Div Error: did not return 404 for invalid id: ${id}`, false)                  
          return {
    ...
    try {      
      ...
      await doPatchDiv('div_name', 'Testing Div * ', 'Testing Div')
      await doNotPatchDiv('div_name', '<script>alert(1)</script>')

      await doPatchDiv('hdcp_per', 0.95, 0.95)
      await doNotPatchDiv('hdcp_per', -1)

      await doPatchDiv('hdcp_from', 222, 222)
      await doNotPatchDiv('hdcp_from', -1)

      await doPatchDiv('int_hdcp', false, false)
      await doNotPatchDiv('int_hdcp', null)

      await doPatchDiv('hdcp_for', 'Series', 'Series')      
      await doNotPatchDiv('hdcp_for', 'abc123')

      await doPatchDiv('sort_order', 5, 5)      
      await doNotPatchDiv('sort_order', -1)

      await divPatchInvalidId('abc_123')
      await divPatchInvalidId('bwl_12345678901234567890123456789012')
    ...
    const invalidDelete = async (invalidId: string) => {      
      try {
        ...
        if (cantDelResponse.status === 404) {
          testResults += addToResults(`Did not not delete Div with invalid id: "${invalidId}"`)
        } else {
          testResults += addToResults(`Error: Could not delete Div with invalid id: "${invalidId}"`, false)
        }
      } catch (error: any) {
        if (error.response.status === 404) {
          testResults += addToResults(`Did not not delete Div - invalid id: "${invalidId}"`)
        } else {
      ...
      if (response.status === 200) {
        ...
        if (divIdToDel === divToDel.id) {
          if (response.data.deleted.hdcp_per_str === divToDel.hdcp_per_str) {
            testResults += addToResults(`Success: Deleted Div: ${divToDel.div_name} - got hdcp_per_str in response`);
          } else {
            testResults += addToResults(`Error Deleted Div: ${divToDel.div_name}: no hdcp_per_str in response`, false);            
            return {
              error: 'No hdcp_per_str in response',
              status: 404,
            }
          }
          testResults += addToResults(`Success: Deleted Div: ${response.data.deleted.div_name}`);          
        }        
      } else {
        testResults += addToResults('False: could not delete div', false)        
        return {
      ...
      if (testing) {
        ...
        try {
          ...
          if (cantDelResponse.status === 409) {
            testResults += addToResults(`Did not not delete div: ${divToUpdate.div_name} with children`)
          } else {
            testResults += addToResults(`Error: Could not delete div: ${divToUpdate.div_name}`, false)            
          }
        } catch (error: any) {
